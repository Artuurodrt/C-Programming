/* Variadic Functions
*
*  Created by Arturo de los Rios on 20/12/21.
*
* C code of how variadic functions are used.
*/

#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>

double average (double v1, double v2, ...);

//Function that calculates the average of two or more arguments
double average (double v1, double v2, ...){

    va_list parg;               //Pointer for a variable
    double sum = v1 + v2;       //Accumulate sum of the
    double value = 0.0;         //Argument value

    int count = 2;              //Count of number of arguments
    va_start(parg, v2);         //Initialize argument

    while((value = va_arg(parg, double)) != 0.0){
        sum += value;
        ++count;
    }

    va_end(parg);

    return sum/count;
}


int main()
{
    double v1 = 10.5, v2 = 2.5;
    int num1 = 6, num2 = 5;
    long num3 = 12L, num4 = 20L;


    printf("Average = %.2lf\n",average(v1, 3.5, v2, 4.5, 0.0));
    printf("Average = %.2lf\n",average(1.0, 2.0, 0.0));
    printf("Average = %.2lf\n",average((double)num2, v2, (double)num1, (double)num4, (double)num3, 0.0) );

    return 0;
}
